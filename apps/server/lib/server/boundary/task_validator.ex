defmodule Server.Boundary.TaskValidator do
  ...
  defp required(errors, fields, required, matcher) do
    with true <- Map.has_key?(fields, required) do
      ...
      false -> [{:error, "#{required} is required"} | errors]
    end
  end
  ...
  defp valid_title(errors, field) when is_binary(field) do
    with true <- String.match?(field, ~r{\S}) do
      ...
      false -> [{:error, "title must not be empty"} | errors]
    end
  end

  defp valid_title(errors, _field), do: [{:error, "title must be a string"} | errors]
end
